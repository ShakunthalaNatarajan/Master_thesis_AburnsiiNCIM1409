...
__usage__ = """
					python transeq.py
					--in <FULL_PATH_TO_INPUT_FILE|INPUT_FOLDER>
					--out <FULL_PATH_TO_OUTPUT_FILE|OUTPUT_FOLDER>
...
def main( arguments ):
	...
	input_spec = arguments[ arguments.index( '--in' )+1 ]
	output_spec = arguments[ arguments.index( '--out' )+1 ]
	...
	if input_spec[-1] == "/":
		input_files = glob.glob( input_file + "*.fasta" ) + glob.glob( input_file + "*.fa" )
	else:
		input_files = [ input_spec ]
	for filename in input_files:
		if output_spec[-1] == "/":
			output_file = output_spec + filename.split('/')[-1].split('.')[0] + ".pep.fa"
		else:
			output_file = output_spec + ""
		sequences = load_multiple_fasta_file( filename )
		print "number of seqs detected for translation: " + str( len( sequences.keys() ) )
